...
void ieee80211_display_current_antenna(struct ieee80211vap *vap)
{
    ...
    if (vap->iv_opmode == IEEE80211_M_HOSTAP)
    {
        TAILQ_FOREACH(ni, &nt->nt_node, ni_list) {
            ...
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_ANY," Tx Antennas for  node: %s > rtset0: %d, rtset1: %d rtset2: %d \n"
                                                             ,ether_sprintf(ni->ni_macaddr), ni->rtset[0].selected_antenna
                                                             ,ni->rtset[1].selected_antenna, ni->rtset[2].selected_antenna);        
        }
    }
}
#endif    
