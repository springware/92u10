...
#ifdef ATH_SWRETRY
        /* To send a frame when tid queues are paused, pspoll flag should be set */
...
#ifdef ATH_SWRETRY
        if ((tid->an->an_flags & ATH_NODE_PWRSAVE) && tid->an->an_pspoll_pending) {
            if (ath_exist_pendingfrm_tidq((ath_dev_t)sc, (ath_node_t)tid->an) ||
    ...
    do {
        ...
        while ( !TAILQ_EMPTY(&ac->tid_q) ){
            ...
            if (tid->paused) {   /* check next tid to keep h/w busy */
                TAILQ_INSERT_TAIL(&paused_tid_q, tid, tid_qelem);
#ifdef ATH_SWRETRY
                /* If node is in PS mode and receives PS-Poll,
                 * schedule the non-empty tid if it is paused.
                 */
                if ((tid->an->an_flags & ATH_NODE_PWRSAVE) && tid->an->an_pspoll_pending && !TAILQ_EMPTY(&tid->buf_q)) {
                    if (ath_aggr_query(tid)) {
                        DPRINTF(sc, ATH_DEBUG_SWR, "%s: Schedule a aggr frame due to pspoll from paused tid (%s) tidno %d qnum %d\n",
                                __func__, TAILQ_EMPTY(&tid->buf_q)?"EMPTY":"NON-EMPTY", tid->tidno, tid->ac->qnum);
                        ath_tx_sched_aggr(sc, txq, tid);
                    }
                    else {
                        DPRINTF(sc, ATH_DEBUG_SWR, "%s: Schedule a normal frame due to pspoll from paused tid (%s) tidno %d qnum %d\n",
                                __func__, TAILQ_EMPTY(&tid->buf_q)?"EMPTY":"NON-EMPTY", tid->tidno, tid->ac->qnum);
                        ath_tx_sched_normal(sc, txq, tid);
                    }

                    DPRINTF(sc, ATH_DEBUG_SWR, "%s: Clear the pspoll_pending\n", __func__);
                    ATH_NODE_SWRETRY_TXBUF_LOCK(tid->an);
