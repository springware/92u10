...
static void ath_adjust_difs (struct ath_softc *sc)
{
    ...
    if (sc->sc_dfs->dfsdomain != DFS_FCC_DOMAIN) {
        return;
    }
...
#define MAX_HT_RATES 24 
    ...
    OS_MEMCPY((u_int32_t *)rate_stats, (u_int32_t *)&an->an_rc_node->txRateCtrl.sm_ratestats[0], MAX_HT_RATES * sizeof(u_int32_t));
    /* Clear the results */
    OS_MEMZERO(&an->an_rc_node->txRateCtrl.sm_ratestats[0], MAX_HT_RATES * sizeof(u_int32_t));
}
