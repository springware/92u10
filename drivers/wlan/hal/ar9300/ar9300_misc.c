...
#define HAL_BB_PANIC_IDLE_TIME_OUT 0x0a8c0000
    ...
    if (idle_tmo_ms != 0) {
        ...
        if (!AR_SREV_WASP(ah)) {
            ...
            /* Don't enable IDLE mode panic for Wasp since 
             * it is causing CAC period detection failures */
            OS_REG_WRITE(ah, AR_PHY_PANIC_WD_CTL_1, 
                     AR_PHY_BB_PANIC_NON_IDLE_ENABLE | 
...
ar9300GetBbPanicInfo(struct ath_hal *ah, struct hal_bb_panic_info *bb_panic)
{
    ...
    if (bb_panic->status == 0x04000539) {
        return -1; //Indicate nothing to return
    }
